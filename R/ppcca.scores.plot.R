 ...
 if(q==1)
  {
	if((group==FALSE)[1])
    {
    	...
    	{
         plot(output$scores[,1], rep(0,nrow(output$scores)), yaxt='n', cex=(Covars[,i]+0.5)*2, pch=16, font=5, xlab="PC 1", ylab="", main=colnames(Covars)[i], col=colr(1, 0.6))
     	...
     	for(i in 1:ncol(Covars))
     	{
         plot(output$scores[,1], rep(0,nrow(output$scores)), yaxt='n', cex=(Covars[,i]+0.5)*2, pch=16, col=colr(group,0.6), font=5, xlab="PC 1", ylab="", main=colnames(Covars)[i])
         legend("topright", bty="n", paste("Variance = ", round(sigma,2), sep=""))
         abline(h=0)
         legend("topleft", bty = "n", paste("Group ", c(gpnames), sep=""), col = names(table(colr(group,0.6))), pch = 16)
         if((ncol(Covars) > 1) & (i < ncol(Covars)))
         {
              ask(msg = "Press <RETURN> to view the scores plot for the next covariate: ")
         }
  ...
  if(q==2)
  {
  	if((group==FALSE)[1])
     {
         ...
         {
               mvn2plot(output$scores[,1:2][j,], sigma[1:2,1:2], k = 15, alone = FALSE, col = "grey50")  
         }
         points(output$scores[,1], output$scores[,2], cex=(Covars[,i]+0.5)*2, pch=16, font=5, col=colr(1,0.6))
         ...
         points(output$scores[,1], output$scores[,2], cex=(Covars[,i]+0.5)*2, pch=16, font=5, col=colr(group,0.6))
         legend("topleft", bty = "n", c("95% Posterior Set"), col = "grey50", lty = c(1), pch = c(3))
          legend("topright", bty = "n", c(paste("Group ", c(gpnames), sep="")), col = names(table(colr(group, 0.6))), lty = rep(0,length(gpnames)),  pch = 16)
          if((ncol(Covars) > 1) & (i < ncol(Covars)))
         {
              ask(msg = "Press <RETURN> to view the scores plot for the next covariate: ")
         }
        ...
        for(i in 1:ncol(Covars))
        {
    		...
    		for(j in 1:choose(q,(q-2)))
        	{
        		...
        		varcov<-matrix(c(sigma[c1,c1], sigma[c1,c2], sigma[c2,c1], sigma[c2,c2]), 2, 2, byrow=TRUE)
          		...
          		points(scores[,1], scores[,2], cex=(Covars[,i]+0.5)*2, pch=16, font=5, col=colr(1, 0.6))
          		legend("topleft", bty = "n", c("95% Posterior Set"), col = "grey50", lty = c(1), pch = c(3))
          		if(j < choose(q,(q-2)))
            	{
       ...
       for(i in 1:ncol(Covars))
        {
    		...
    		for(j in 1:choose(q,(q-2)))
        	{
        		...
        		varcov<-matrix(c(sigma[c1,c1], sigma[c1,c2], sigma[c2,c1], sigma[c2,c2]), 2, 2, byrow=TRUE)
          		...
          		points(scores[,1], scores[,2], cex=(Covars[,i]+0.5)*2, pch=16, font=5, col=colr(group, 0.6))
          		legend("topleft", bty = "n", c("95% Posterior Set"), col = "grey50", lty = c(1), pch = c(3))
          		 legend("topright", bty = "n", paste("Group ", c(gpnames), sep=""), lty = rep(0,length(gpnames)), col = names(table(colr(group, 0.6))), pch = 16)
          		if(j < choose(q,(q-2)))
            	{
              		ask(msg = "Press <RETURN> to view the scores plot for the next pair of dimensions: ")
         		}
